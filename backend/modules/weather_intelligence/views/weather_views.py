...
class WeatherSnapshotViewSet(viewsets.GenericViewSet):
    ...
    def get_queryset(self):
        ...
        if not user or not user.is_authenticated:
            return self.weather_service.weather_repository.model_class.active_objects.none()
        if getattr(user, "role", "") == "admin" or user.is_superuser:
            return self.weather_service.weather_repository.model_class.active_objects.all()
        return self.weather_service.weather_repository.model_class.active_objects.filter(farm__owner=user)
    ...
    def retrieve(self, request: Request, pk: str = None) -> Response:
        ...
        return success_response(data=serializer.data, message="Weather snapshot retrieved successfully.")
    ...
    def update(self, request: Request, pk: str = None) -> Response:
        ...
        updated_snapshot = self.weather_service.update_snapshot(snapshot_id=pk, **serializer.validated_data)
    ...
    def destroy(self, request: Request, pk: str = None) -> Response:
        ...
        return success_response(data={}, message="Weather snapshot deleted successfully.", status_code=status.HTTP_200_OK)
