...
class DashboardService:
    def get_dashboard_overview(self, user):
        ...
        ).exclude(predicted_class__icontains="healthy").order_by('-created_at')[:5]

        # Recommendations
        recent_crop_recs = CropRecommendation.objects.filter(user=user).order_by('-created_at')[:5]
        recent_fert_recs = FertilizerRecommendation.objects.filter(user=user).order_by('-created_at')[:5]

        # Notifications (Actionable Tasks)
        actionable_notifications = Notification.objects.filter(
            user=user, 
            is_read=False,
            notification_type__in=[Notification.NotificationType.WARNING, Notification.NotificationType.CRITICAL]
        ).order_by('-created_at')[:5]

        # Actionable insights generation
        insights = []
        if active_farms == 0:
            insights.append({"type": "info", "message": "You haven't added any farms yet. Add a farm to start receiving tailored insights."})
        
        if recent_diseases.exists():
            latest = recent_diseases.first()
            disease_name = latest.predicted_class.replace('___', ' - ').replace('_', ' ')
            insights.append({
                "type": "warning", 
