...
from core.permissions import IsAdmin, AdminOrReadOnly
...
@extend_schema_view(
    ...
    create=extend_schema(description="Add a new crop to the master directory (Admin only)."),
)
class CropViewSet(viewsets.GenericViewSet):
    ...
    def get_paginated_response(self, data):
        return paginated_response(self.paginator, data, message="Crops retrieved successfully.")
    ...
    def list(self, request: Request) -> Response:
        ...
        return success_response(data=serializer.data, message="Crops retrieved successfully.")

    def retrieve(self, request: Request, pk: str = None) -> Response:
        crop = self.crop_service.get_crop(crop_id=pk)
        serializer = self.get_serializer(crop)
        return success_response(data=serializer.data, message="Crop details retrieved successfully.")
    ...
    def update(self, request: Request, pk: str = None) -> Response:
        serializer = self.get_serializer(data=request.data)
        serializer.is_valid(raise_exception=True)
        updated_crop = self.crop_service.update_crop(crop_id=pk, **serializer.validated_data)
        return success_response(
    ...
    def destroy(self, request: Request, pk: str = None) -> Response:
        ...
        return success_response(data={}, message="Crop deleted successfully.", status_code=status.HTTP_200_OK)
