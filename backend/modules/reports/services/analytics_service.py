...
class AnalyticsService:
    def get_analytics(self, user):
        ...
        disease_freq = list(
            diseases_qs.values("predicted_class")
        ...
        for d in disease_freq:
            d["predicted_class"] = (
                d["predicted_class"].replace("___", " - ").replace("_", " ")
            )
        ...
        if disease_freq:
            ...
            if "healthy" not in most_common["predicted_class"].lower():
                insights.append(
                    f"Your most frequently detected issue is {most_common['predicted_class']} ({most_common['count']} times). Consider applying preventative treatments across your fields."
                )
            else:
                insights.append(
                    f"Great news! Your most frequent scan result is Healthy ({most_common['count']} times)."
                )

        total_scans_last_month = sum(
            [m["count"] for m in monthly_scans if m["month"] == now.strftime("%b %Y")]
        )
