...
class ChatViewSet(viewsets.GenericViewSet):
    ...
    def list(self, request: Request) -> Response:
        ...
        return success_response(
            data=serializer.data, message="Chat sessions retrieved."
        )
    ...
    def retrieve(self, request: Request, pk: str = None) -> Response:
        ...
        if not session:
            return error_response(
                message="Session not found.", status_code=status.HTTP_404_NOT_FOUND
            )
        serializer = self.get_serializer(session)
        return success_response(data=serializer.data, message="Chat session retrieved.")

    @extend_schema(
        description="Send a message to the AI assistant. (Deprecated: use /chat/ instead)"
    )
    ...
    def chat(self, request: Request) -> Response:
        ...
        try:
            result = service.process_message(
                user=request.user, session_id=session_id, message=message
            )
            return success_response(
                data=result, message="Message processed successfully."
            )
