...
class RuleBasedProvider(BaseProvider):
    def generate_response(
        ...
        if "disease" in text or "scan" in text or "sick" in text:
            recent = (
                DiseasePrediction.active_objects.filter(user=user)
                .order_by("-created_at")
                .first()
            )
            if recent:
                ...
                diagnosis = Disease.active_objects.filter(
                    name__icontains=recent.predicted_class
                ).first()
                if diagnosis:
                    treatments = diagnosis.treatments.all()
                    treatment_txt = "\n".join(
                        [f"- {t.instruction}" for t in treatments]
                    )
        ...
        if "fertilizer" in text or "nutrient" in text or "soil" in text:
            recent = (
                FertilizerRecommendation.active_objects.filter(user=user)
                .order_by("-created_at")
                .first()
            )
            if recent:
        ...
        if "what is" in text or "how to treat" in text or "about" in text:
            ...
            for d in diseases:
                if d.name.lower() in text or any(
                    word in d.name.lower() for word in words if len(word) > 4
                ):
