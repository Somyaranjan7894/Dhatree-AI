...
def _load_joblib_model_and_encoder(registry_path):
    ...
    model_path = (
        ...
        / "fertilizer_production_best.joblib"
    )
    encoder_path = (
        registry_path / "fertilizer_recommendation" / "fertilizer_label_encoder.joblib"
    )
...
class FertilizerRecommendationPipeline(PredictorInterface):
    ...
    def __init__(self) -> None:
        ...
        self.encoder = None
        try:
            self.model, self.encoder = self.registry.get_model(
                self.MODEL_KEY, _load_joblib_model_and_encoder
            )
        except Exception as e:
            logger.warning(
                f"Fertilizer model failed to load. Will fall back to mock. Error: {e}"
            )
