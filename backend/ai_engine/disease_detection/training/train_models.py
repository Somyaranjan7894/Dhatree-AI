...
from ai_engine.disease_detection.utils.dataset_utils import load_plantvillage_dataset, compute_class_weights
...
def train_production(epochs: int = 2, batch_size: int = 8):
    ...
    for model_name in models_to_train:
        ...
        poor_classes = [class_names[i] for i, acc in enumerate(per_class_acc) if acc < 0.5]
        if poor_classes:
            print(f"Classes with poor performance (<50%): {poor_classes}")
        
        combined_score = (top1_acc + f1) / 2
        print(f"Model: {model_name} | Top-1 Acc: {top1_acc:.4f} | Top-3 Acc: {top3_acc:.4f} | F1: {f1:.4f} | Precision: {precision:.4f} | Recall: {recall:.4f}")
