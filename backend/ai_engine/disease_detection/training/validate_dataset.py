...
def validate_plantvillage_dataset(dataset_dir: str):
                ...
                with open(file_path, "rb") as image_file:
                    ...
                    if file_hash in image_hashes:
                        duplicate_images.append(
                            (str(file_path), image_hashes[file_hash])
                        )
                    else:
    ...
    if empty_folders:
        print(
            f"[FAILED] Empty Folders Detected ({len(empty_folders)}): {empty_folders}"
        )
        issues_found = True
    else:
    ...
    if unsupported_images:
        print(
            f"[FAILED] Unsupported Images ({len(unsupported_images)} found). Example: {unsupported_images[:3]}"
        )
        issues_found = True
    else:
    ...
    if corrupted_images:
        print(
            f"[FAILED] Corrupted Images ({len(corrupted_images)} found). Example: {corrupted_images[:3]}"
        )
        issues_found = True
    else:
    ...
    if duplicate_images:
        print(
            f"[FAILED] Duplicate Images ({len(duplicate_images)} found). Example: {duplicate_images[:3]}"
        )
        issues_found = True
    else:
