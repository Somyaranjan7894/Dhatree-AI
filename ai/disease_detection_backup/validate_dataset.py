...
RAW_DATA_DIR = Path("ai/datasets/raw/plantvillage")
...
def validate_dataset():
    ...
    if len(subdirs) == 1 and subdirs[0].name == "PlantVillage":
        ...
        base_dir = base_dir / "raw" / "color" if (base_dir / "raw" / "color").exists() else base_dir
    else:
    ...
    if report["classes"]:
        ...
        if max_count > min_count * 10:
            report["issues"].append("Severe class imbalance detected (>10x difference between max and min class)")
            
    METADATA_DIR.mkdir(parents=True, exist_ok=True)
    with open(REPORT_PATH, "w") as f:
        json.dump(report, f, indent=4)
        
    logging.info(f"Validation complete. Found {report['total_images']} valid images across {len(report['classes'])} classes.")
